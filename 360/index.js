...
(function() {
  ...
  if (globeToggleElement) {
    globeToggleElement.addEventListener('click', function() {
      mapPopupElement.classList.toggle('visible');
      if (mapOverlayElement) mapOverlayElement.classList.toggle('visible');
    });
  }

  window.addEventListener('click', function(e) {
    if (mapPopupElement.classList.contains('visible') &&
        !mapPopupElement.contains(e.target) &&
        (!globeToggleElement || !globeToggleElement.contains(e.target))) {
      mapPopupElement.classList.remove('visible');
      if (mapOverlayElement) mapOverlayElement.classList.remove('visible');
    }
  });

  var mapWaypoints = document.querySelectorAll('.map-waypoint');
  mapWaypoints.forEach(function(wp) {
  ...
  var z_vel = 0.5;
  var t_vel = 22.5 * Math.PI / 180;
  var friction = 3;

  // Associate view controls with elements.
  var controls = viewer.controls();
  controls.registerMethod('upElement',    new Marzipano.ElementPressControlMethod(viewUpElement,     'y', -t_vel, friction), true);
  controls.registerMethod('downElement',  new Marzipano.ElementPressControlMethod(viewDownElement,   'y',  t_vel, friction), true);
  controls.registerMethod('leftElement',  new Marzipano.ElementPressControlMethod(viewLeftElement,   'x', -t_vel, friction), true);
  ...
  function switchScene(scene) {
    ...
    scene.scene.switchTo({ transitionDuration: 300 });
    startAutorotate();
  ...
  function createInfoHotspotElement(hotspot) {
    ...
    closeIcon.src = './img/expand_info.png';
